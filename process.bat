...
set conf=%app%\conf
set rclone-config=%app%\conf\rclone-%modpack-version%.conf
...
IF not EXIST %rclone-config% (
    bitsadmin.exe /transfer Job "https://raw.githubusercontent.com/savvasdalkitsis/minecraft-modpack-kevin-nation/%modpack-version%/rclone.conf" %rclone-config%
)
